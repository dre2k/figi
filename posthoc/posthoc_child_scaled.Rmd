...
rawHTML <- paste(readLines(glue("{path}/output/posthoc/gxe_models_{exposure}_scaled_{hrc_version}_{snp}_dose_{glue_collapse(sort(covariates), sep = '_')}_stratified_by_sex.html")))
...
rawHTML <- paste(readLines(glue("{path}/output/posthoc/gxe_models_{exposure}_scaled_{hrc_version}_{snp}_dose_{glue_collapse(sort(covariates), sep = '_')}_stratified_by_cancer_site_sum2.html")))
cat(paste(rawHTML, collapse = "\n"), "\n")
```
...
rawHTML <- paste(readLines(glue("{path}/output/posthoc/gxe_models_{exposure}_scaled_{hrc_version}_{snp}_dose_{glue_collapse(sort(covariates), sep = '_')}_stratified_by_study_design.html")))
cat(paste(rawHTML, collapse = "\n"), "\n")
```
...
cat("Variable is scaled by the median sex/study specific IQR, THEN centered around median / IQR (e.g. ref = median, comparison group = median + 1IQR)
")

#### ORIGINAL (GENOTYPES)
stratified_or_file <- readRDS(glue("{path}/output/posthoc/stratified_oddsratio_{exposure}_scaled_{hrc_version}_{snp}_{glue_collapse(sort(covariates), sep = '_')}.rds"))

# stratified_or_file <- readRDS("/media/work/gwis_test/redmeatqcm_v2/output/posthoc/stratified_oddsratio_redmeatqcm_v2_v3.0_chr18_46453754_C_T_age_ref_imp_energytot_imp_pc1_pc2_pc3_sex_study_gxe.rds")
...
interactionplot_file <- glue("{path}/output/posthoc/interaction_plot_{exposure}_scaled_{hrc_version}_{snp}_dose_{glue_collapse(sort(covariates), sep = '_')}.png")
