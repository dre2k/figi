...
model <- glm(outcome ~ alcoholc_moderate*chr10_101476905_G_A + age_ref_imp + sex + energytot_imp + pc1 + pc2 + pc3 + study_gxe, data = figi, family = 'binomial')
...
values <- data.frame(alcoholc_moderate = c(0,0), 
                     chr10_101476905_G_A = c(0,2), 
                     age_ref_imp = c(0,0), 
                     energytot_imp = c(0,0),
                     sex = c(1,1), 
                     pc1 = c(0,0),
...
interact_plot(model, modx = 'alcoholc_moderate' , pred = 'chr10_101476905_G_A', plot.points = F, interval = T, outcome.scale = 'response', y.label = 'predicted log odds')
sim_margins(model, modx = 'alcoholc_moderate' , pred = 'chr10_101476905_G_A')



huh <- wtf(model, modx = 'alcoholc_moderate' , pred = 'chr10_101476905_G_A', outcome.scale = 'link')$predicted %>% 
  filter()
...
wtf <- function (model, pred, modx, modx.values = NULL, mod2 = NULL, 
                 mod2.values = NULL, centered = "all", data = NULL, at = NULL, 
                 ...
                 colors = NULL, line.thickness = 1, vary.lty = TRUE, point.size = 1.5, 
                 point.shape = FALSE, jitter = 0, rug = FALSE, rug.sides = "b", 
                 partial.residuals = FALSE, point.alpha = 0.6, color.class = NULL, 
                 ...) 
{
  dots <- list(...)
  if (length(dots) > 0) {
  ...
  pred_out <- prep_data(model = model, pred = pred, modx = modx, 
                        ...
                        facet.modx = facet.modx, d = d, survey = "svyglm" %in% 
                          class(model), weights = weights, preds.per.level = 100, 
