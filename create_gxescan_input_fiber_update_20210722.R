...
vcfid_fiber <- readRDS("/media/work/gwis_test/fiberqc2/data/FIGI_v2.3_gxeset_fiberqc2_basic_covars_glm.rds") %>% 
  pull(vcfid) # original N = 76157
study_fiber <- readRDS(glue("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_analysis_data_glm.rds")) %>% 
  pull(study) %>% unique(.)

input_data <- readRDS(glue("/media/work/gwis/data/FIGI_EpiData/FIGI_{hrc_version}_gxeset_analysis_data_glm.rds")) %>% 
  filter(study %in% study_fiber, 
         !is.na(fiber10qcm)) %>% 
...
out <- dplyr::filter(input_data, is.na(adenoma),
                     !study_gxe %in% c("SMS_AD", "REACH_AD", "HawaiiCCS_AD", "HPFS_3_AD", "NHS_3_AD", "NHS_5_AD", "PLCO_4_AD", "PPS3", "PPS4"))
table(out$study_gxe, out$outc, useNA = 'ifany')
