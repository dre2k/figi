...
# ------ HRT ver 2------
# PURE EO and EP variables (11/30/2020) --------# 
# https://drive.google.com/file/d/1WoUnA_mh6oKb6UAC6-X_-MLEx47yAZ6r/view?usp=sharing
#
# HawaiiCCS has updated data but still high missingness for HRT. exclude for now
...
# table(figi_tmp$ep_ref_pm_gxe)
# 
# table(figi_tmp$outcome, figi_tmp$pure_ep_allNo)
# table(figi_tmp$study_gxe, figi_tmp$pure_ep_allNo)
# table(figi_tmp$outcome, figi_tmp$pure_ep_allNo, figi_tmp$study_gxe)
# 
# # eo_ref_pm_gxe (see above)
# exclude = c("HawaiiCCS_AD", "REACH_AD", "SMS_AD")
# wrap(figi_tmp, 'pure_eo_allNo', vars_to_exclude = c("energytot_imp", "sex"), studies_to_exclude = exclude)
# 
# x <- readRDS("~/data/GxEScanR_PhenotypeFiles/FIGI_v2.3_gxeset_pure_eo_allNo_basic_covars_glm.rds")
# table(x$study_gxe, x$outcome)
# y <- readRDS("~/data/GxEScanR_PhenotypeFiles/FIGI_v2.3_gxeset_pure_eo_allNo_basic_covars_gxescan.rds")
# 
# 
# # ep_ref_pm_gxe (see above)
# exclude = c("HawaiiCCS_AD", "REACH_AD", "SMS_AD")
# wrap(figi_tmp, 'pure_ep_allNo', vars_to_exclude = c("energytot_imp", "sex"), studies_to_exclude = exclude)
# x <- readRDS("~/data/GxEScanR_PhenotypeFiles/FIGI_v2.3_gxeset_pure_ep_allNo_basic_covars_glm.rds")
# table(x$study_gxe, x$outcome)
# 
# 
# wrap(figi_gwas, 'ep_ref_pm_gxe', vars_to_exclude = c("energytot_imp", "sex"))
...
# ---- update ------ #
# use Yi's subset to re-run pure_eo_all, and pure_ep_all (and possibly hrt_ref_pm2 just in case.. but probably not)

# first, let's compare hrt_ref_pm2 between mine and Yi's 

hrt_ref_pm2_ak <- readRDS("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_hrt_ref_pm2_basic_covars_glm.rds")
load("/media/work/gwis/data/FIGI_EpiData/Data_HRT_USC_epi_v2.3-20201204.Rdata")

hrt_ref_pm2_yi <- data.usc %>% 
  filter(!is.na(hrt_ref_pm2))

anti <- anti_join(hrt_ref_pm2_yi, hrt_ref_pm2_ak , 'vcfid')
anti <- anti_join(hrt_ref_pm2_ak, hrt_ref_pm2_yi , 'vcfid')


# pure_eo_all

# pure_eo_all_ak <- readRDS("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_eo_allNo_basic_covars_glm.rds")
# pure_eo_all_yi <- data.usc %>% 
#   filter(!is.na(pure_eo_all))
# 
# table(pure_eo_all_ak$study_gxe, pure_eo_all_ak$outcome)
# table(pure_eo_all_yi$study_gxe, pure_eo_all_yi$outc)
# 
# table(pure_eo_all_ak$study_gxe, pure_eo_all_ak$pure_eo_all)
# table(pure_eo_all_yi$study_gxe, pure_eo_all_yi$pure_eo_all)

pure_eo_all_yi <- data.usc %>% 
  filter(!is.na(pure_eo_all))

sss <- inner_join(figi_gxe, pure_eo_all_yi[, c('vcfid', 'pure_eo_all')], 'vcfid')

tmp <- sss %>% 
  mutate(pure_eo_all = as.numeric(pure_eo_all),
         outcome = ifelse(outc == "Case", 1, 
                   ifelse(outc == "Control", 0, NA)))

drops <- data.frame(table(tmp$outcome, tmp[, 'pure_eo_all'], 
                          tmp$study_gxe)) %>% 
  filter(Freq <= 1)

tmp <- filter(tmp, !study_gxe %in% unique(drops$Var3)) %>% 
  mutate(study_gxe = fct_drop(study_gxe)) %>% 
  dplyr::select(vcfid, outcome, pure_eo_all, study_gxe, age_ref_imp, PC1, PC2, PC3) %>% 
  filter(complete.cases(.))

saveRDS(tmp, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_eo_all_basic_covars_glm.rds"), version = 2)
cov_gxescan <- format_data_gxescan(tmp, "pure_eo_all")
saveRDS(cov_gxescan, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_eo_all_basic_covars_gxescan.rds"), version = 2)
...
# ----- pure_ep_all 
# 
pure_ep_all_yi <- data.usc %>% 
filter(!is.na(pure_ep_all))

sss <- inner_join(figi_gxe, pure_ep_all_yi[, c('vcfid', 'pure_ep_all')], 'vcfid')

tmp <- sss %>% 
  mutate(pure_ep_all = as.numeric(pure_ep_all),
         outcome = ifelse(outc == "Case", 1, 
                          ifelse(outc == "Control", 0, NA)))

drops <- data.frame(table(tmp$outcome, tmp[, 'pure_ep_all'], 
                          tmp$study_gxe)) %>% 
  filter(Freq <= 1)

tmp <- filter(tmp, !study_gxe %in% unique(drops$Var3)) %>% 
  mutate(study_gxe = fct_drop(study_gxe)) %>% 
  dplyr::select(vcfid, outcome, pure_ep_all, study_gxe, age_ref_imp, PC1, PC2, PC3) %>% 
  filter(complete.cases(.))

saveRDS(tmp, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_ep_all_basic_covars_glm.rds"), version = 2)
cov_gxescan <- format_data_gxescan(tmp, "pure_ep_all")
saveRDS(cov_gxescan, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_ep_all_basic_covars_gxescan.rds"), version = 2)



# MAKE SURE COUNTS ARE OK
x <- readRDS("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_eo_all_basic_covars_gxescan.rds")
nrow(x)
x <- readRDS("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_ep_all_basic_covars_gxescan.rds")
nrow(x)
...
sss <- filter(figi_gxe, vcfid %in% hrt_ref_pm2_yi$vcfid)
...
drops <- data.frame(table(tmp$outcome, tmp[, 'hrt_ref_pm2'], 
                          tmp$study_gxe)) %>% 
  filter(Freq <= 1)

tmp <- filter(tmp, !study_gxe %in% unique(drops$Var3)) %>% 
  mutate(study_gxe = fct_drop(study_gxe)) %>% 
  dplyr::select(vcfid, outcome, hrt_ref_pm2, study_gxe, age_ref_imp, PC1, PC2, PC3) %>% 
  filter(complete.cases(.))

table(tmp$study_gxe, tmp$hrt_ref_pm2)

# IT'S JUST A DIFFERENCE OF 5 PEOPLE, NO NEED TO RUN =)

# saveRDS(tmp, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_ep_all_basic_covars_glm.rds"), version = 2)
# cov_gxescan <- format_data_gxescan(tmp, "hrt_ref_pm2")
# saveRDS(cov_gxescan, file = paste0("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_pure_ep_all_basic_covars_gxescan.rds"), version = 2)
...
# within MCCS, 4 discrepancy either no calcium_tot or not EUR
...
wrap <- function(d, exposure, is_e_categorical = T, min_cell_size = 0, vars_to_exclude = c("energytot_imp"), vars_to_include = c(), studies_to_exclude) {
  cov <- format_data_glm(d, exposure, is_e_categorical, min_cell_size, vars_to_exclude, vars_to_include, eur_only = T) %>% 
    filter(!study_gxe %in% studies_to_exclude) %>% 
    mutate(study_gxe = fct_drop(study_gxe))
  saveRDS(cov, file = paste0("/media/work/gwis/results/input/FIGI_v2.3_gxeset_", exposure, "_basic_covars_glm.rds"), version = 2)
  
  cov_gxescan <- format_data_gxescan(cov, exposure)
  saveRDS(cov_gxescan, file = paste0("/media/work/gwis/results/input/FIGI_v2.3_gxeset_", exposure, "_basic_covars_gxescan.rds"), version = 2)
}
...
y <- readRDS("/media/work/gwis/results/input/FIGI_v2.3_gxeset_height10_basic_covars_glm.rds")
table(x$study_gxe, x$outcome)
table(x$study_gxe, x$bmi5)

## 06/04/2020 edit
## run separate scans for cohort and case-control studies
wrap <- function(d, exposure, is_e_categorical = T, min_cell_size = 0, vars_to_exclude = c("energytot_imp"), vars_to_include = c(), studies_to_exclude, suffix) {
  cov <- format_data_glm(d, exposure, is_e_categorical, min_cell_size, vars_to_exclude, vars_to_include, eur_only = T) %>% 
    filter(!study_gxe %in% studies_to_exclude) %>% 
    mutate(study_gxe = fct_drop(study_gxe))
  saveRDS(cov, file = paste0("~/data/results/input/FIGI_v2.3_gxeset_", exposure, "_basic_covars_glm_", suffix, ".rds"), version = 2)
  
  cov_gxescan <- format_data_gxescan(cov, exposure)
  saveRDS(cov_gxescan, file = paste0("~/data/results/input/FIGI_v2.3_gxeset_", exposure, "_basic_covars_gxescan_", suffix, ".rds"), version = 2)
}

x <- readRDS("~/data/GxEScanR_PhenotypeFiles/FIGI_v2.3_gxeset_height10_basic_covars_glm.rds") %>% 
  pull(vcfid)
figi <- readRDS(paste0("~/data/results/input/FIGI_", "v2.3", "_gxeset_analysis_data_glm.rds")) %>% 
  filter(vcfid %in% x)
table(figi$study_design)


x_cohort <- filter(figi, study_design == "Cohort") %>% 
  rename_at(vars(starts_with('pc')), toupper) %>% 
  mutate(outcome = ifelse(outcome == 0, 'Control', 'Case'),
         sex = ifelse(sex == 0, "Female", "Male"))
exclude = c("")
wrap(x_cohort, exposure = 'height10', is_e_categorical = F, vars_to_exclude = c("energytot_imp"), studies_to_exclude = exclude, suffix = "cohort")
z <- readRDS("~/data/results/input/FIGI_v2.3_gxeset_height10_basic_covars_glm_cohort.rds")
table(z$study_gxe, z$outcome)
table(z$study_gxe, z$sex)


x_casecontrol <- filter(figi, study_design == "Case-Control") %>% 
  rename_at(vars(starts_with('pc')), toupper) %>% 
  mutate(outcome = ifelse(outcome == 1, 'Case', 'Control'),
         sex = ifelse(sex == 0, "Female", "Male"))
exclude = c("")
wrap(x_casecontrol, exposure = 'height10', is_e_categorical = F, vars_to_exclude = c("energytot_imp"), studies_to_exclude = exclude, suffix = "casecontrol")
zz <- readRDS("~/data/results/input/FIGI_v2.3_gxeset_height10_basic_covars_glm_casecontrol.rds")


check <- readRDS("~/data/results/input/FIGI_v2.3_gxeset_height10_basic_covars_gxescan_cohort.rds")
check <- readRDS("~/data/results/input/FIGI_v2.3_gxeset_height10_basic_covars_gxescan_casecontrol.rds")
