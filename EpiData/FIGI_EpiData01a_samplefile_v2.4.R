...
samplefile = read.table('Samplefile_HRC1.1_epi_v2.4-20200303_usc.txt', header=T, fileEncoding='latin1', stringsAsFactors= F, sep='\t', na.strings=c('',NA))
...
table(samplefile$gxe, samplefile$EUR_subset) # 99038
...
check <- inner_join(samplefile, vcf_samplenames, by = "vcfid") # discrepancy of 1727
check <- anti_join(samplefile, vcf_samplenames, by = "vcfid") # 1738 in samplefile only 
check <- anti_join(vcf_samplenames, samplefile, by = "vcfid") # 11 in vcf files only - (that's ok, previously discussed)
...
save.image("/media/work/gwis/data/FIGI_EpiData/FIGI_SampleFile_Summary_200309.RData")
