...
} else if(hrc_version == "v3.1") {
  load("/media/work/gwis_test/data/HRCv3.1/FIGI_Genotype_Epi_211116.RData")
}
...
# merge in physact variable
load("/media/work/gwis_test/data/physact_additional_vars/PA_Dataset.Rdata")
figi_gwas <- left_join(figi_gwas, gxe[, c('vcfid', 'methrswklnsqc2', 'vigmodlnsqc2')], 'vcfid')
...
} else if(hrc_version %in% c("v3.0", "v3.1")) {
...
# merge in physact variable
load("/media/work/gwis_test/data/physact_additional_vars/PA_Dataset.Rdata")
figi_gxe <- left_join(figi_gxe, gxe[, c('vcfid', 'methrswklnsqc2', 'vigmodlnsqc2')], 'vcfid')
