...
# FIGI Analysis 03/09/2020 Update v2.4
# FIGI Analysis 03/09/2020 Update v3.0 (in FIGI, only using for total folate and total calcium re-do)
#
# FIGI Analyis 11/16/2021 Update - create V3.1 for PhysAct analysis
#
#
...
#
# new drops for HRC v3.0
#
#
#-----------------------------------------------------------------------------#
# samplefile <- fread("Samplefile_HRC1.1_epi_v2.3-20191205_usc.txt")
samplefile = read.table('Samplefile_HRC1.1_epi_v3.1_usc-20210217.txt', header=T, fileEncoding='latin1', stringsAsFactors= F, sep='\t', na.strings=c('',NA))
...
table(samplefile$gxe, samplefile$EUR_subset) # 99726 (same N as V3.0)
...
check <- inner_join(samplefile, vcf_samplenames, by = "vcfid") # 1727 discrepancy
check <- anti_join(samplefile, vcf_samplenames, by = "vcfid") # 1738 in samplefile only 
check <- anti_join(vcf_samplenames, samplefile, by = "vcfid") # 11 in vcf files only - (that's ok, already discussed)
...
# save.image("~/data/FIGI_EpiData_rdata/FIGI_SampleFile_Summary_200309.RData")
# new location below
save.image(glue("{output_dir}/FIGI_SampleFile_Summary_211116.RData"))
