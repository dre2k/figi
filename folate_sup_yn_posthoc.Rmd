...
# 3df finding
(main driven by E|G component - as you can see there's very little indication of GxE interaction)
```{r, results = 'asis'}
# child method variable
assign("method", "chiSq3df", child_env)

# SNPs
# snps_clumped <- fread(glue("{path}/data/FIGI_{hrc_version}_gxeset_{exposure}_chiSq3df_no_gwas_ldclump.clumped"), header = T, stringsAsFactors = F)
# snps_out <- readRDS(glue("{path}/output/twostep_liji_{exposure}_{hrc_version}_chiSqG_df.rds")) %>%
#   dplyr::arrange(Chromosome, Location) %>%
#   dplyr::mutate(snps = paste0('chr', gsub("\\:", "\\_", SNP))) %>%
#   dplyr::pull(snps)

snps_out <- c("chr8_68909017_T_G")
