...
wrap <- function(d, exposure, is_e_categorical = T, min_cell_size = 0, vars_to_exclude = c("energytot_imp"), vars_to_include = c(), studies_to_exclude) {
  ...
  saveRDS(cov, file = paste0("~/data/results/input/FIGI_v2.4_gxeset_", exposure, "_basic_covars_glm.rds"), version = 2)
  
  cov_gxescan <- format_data_gxescan(cov, exposure)
  saveRDS(cov_gxescan, file = paste0("~/data/results/input/FIGI_v2.4_gxeset_", exposure, "_basic_covars_gxescan.rds"), version = 2)
}
...
x <- readRDS("~/data/gwis_v2.4/input/FIGI_v2.4_gxeset_asp_ref_basic_covars_glm.rds")
...
wrap(figi_gwas, 'nsaids', studies_to_exclude = exclude)
x <- readRDS("~/data/gwis_v2.4/input/FIGI_v2.4_gxeset_nsaids_basic_covars_glm.rds")
table(x$study_gxe, x$outcome)
...
# aspirin (no mutual adjustment for other NSAIDs variable)
...
x <- readRDS("~/data/GxEScanR_PhenotypeFiles/FIGI_v2.4_gxeset_height10_basic_covars_glm.rds")
table(x$study_gxe, x$outcome)
table(x$study_gxe, x$bmi5)
