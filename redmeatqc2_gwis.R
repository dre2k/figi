...
snps_filter <- snps_3df[c(17, 21, 22, 27, 28, 8, 9)]


posthoc_run_models(snps_filter, 'chiSq3df', quartile = T)
posthoc_run_reri_plot(snps_filter)
posthoc_create_plots(snps_3df, 'chiSq3df')


# ------- case only ------- #
snps_filter <- readRDS(glue(output_dir, "manhattan_chiSqCase_{exposure}_clump_df.rds")) %>% 
  dplyr::mutate(snps = paste0('chr', gsub("\\:", "\\_", SNP))) %>% 
  pull(snps)

posthoc_run_models(snps_filter, 'chiSqCase', quartile = T)
posthoc_run_reri_plot(snps_filter)
posthoc_create_plots(snps_filter, 'chiSqCase')


# ------- two-step methods ------- #
# twostep_wht_chiSqG_redmeatqc2_df.rds
snps_filter <- readRDS(glue(output_dir, "twostep_wht_chiSqG_{exposure}_df.rds")) %>% 
  dplyr::mutate(snps = paste0('chr', gsub("\\:", "\\_", SNP))) %>% 
  pull(snps)
