...
exposure_subset <- readRDS(paste0(output_dir, "/FIGI_EpiData/FIGI_", params$hrc_version, "_gxeset_", params$exposure, "_basic_covars_glm.rds"))[, 'vcfid']
...
if (params$exposure %in% c("folate_totqc2", "folate_dietqc2", "bmi5")) {
  gxe_ggplot <-  readRDS(paste0(output_dir, "/FIGI_EpiData/FIGI_", params$hrc_version, "_gxeset_analysis_data_ggplot.rds"))
  gxe_ggplot_subset <- gxe_ggplot %>%
    filter(vcfid %in% exposure_subset) %>% 
    mutate({{exposure_tmp}} := abs(.data[[exposure_tmp]] - 3))
  
  gxe_table1 <-  readRDS(paste0(output_dir, "/FIGI_EpiData/FIGI_", params$hrc_version, "_gxeset_analysis_data_table1.rds"))
