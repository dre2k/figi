...
vcfid_fruit <- readRDS("/media/work/gwis_test/fruitqc2/data/FIGI_v2.3_gxeset_fruitqc2_basic_covars_glm.rds") %>% 
  pull(vcfid) # original N = 76014
study_fruit <- readRDS(glue("/media/work/gwis/data/FIGI_EpiData/FIGI_v2.3_gxeset_analysis_data_glm.rds")) %>% 
  pull(study) %>% unique(.)

input_data <- readRDS(glue("/media/work/gwis/data/FIGI_EpiData/FIGI_{hrc_version}_gxeset_analysis_data_glm.rds")) %>% 
  filter(study %in% study_fruit, 
         !is.na(fruit5qcm)) %>% 
...
wrap(d = out, exposure = exposure, hrc_version = hrc_version, is_e_categorical = F, path = '/media/work/gwis_test/fruit5qcm/data/', studies_to_exclude = "")
...
model <- glm(outcome ~ fruit5qcm + age_ref_imp + energytot_imp + sex + study_gxe, data = check, family = 'binomial')
summary(model)
exp(coef(model))
