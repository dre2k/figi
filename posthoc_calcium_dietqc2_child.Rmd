...
out <- readRDS(paste0(outdir, "pvalues_dataframe_", method, "_", snp, "_", exposure, ".rds")) %>%
...
rawHTML <- paste(readLines(paste0(outdir, "gxe", "_", method, "_", snp, "_", exposure, "_", "covariate_sets.html")))
...
x <- readRDS(paste0(outdir, 'stratified_oddsratio_', snp, '_', exposure, '.rds'))
...
kable(x) %>%
  ...
  pack_rows("E param by G", 9, 16, indent = F) %>%
