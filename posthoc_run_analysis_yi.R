...
# I'm not sure if this is issue with binarydosage package.. but 
# sometimes get duplicate regions... 
# e.g. (ep_ref_pm_gxe)
# identical(dat$X1.53785007.C.T_p2, dat$X1.53785007.C.T_p2.1)
...
#-----------------------------------------------------------------------------#
# output data.frame of MAFs.. then merge with list of SNPs in rmarkdown ----
#-----------------------------------------------------------------------------#
...
maf_out <- dat %>% 
  filter(outcome == 0) %>% 
  mutate_at(vars(snp_list), function(x) round(sum(x) / (nrow(.)*2), 3)) %>% 
  dplyr::select(snp_list) %>% 
  .[1,] %>% 
  pivot_longer(everything())

tmpsnp <- gsub("X", "chr", maf_out$name) %>% 
          gsub("_dose", "", .) %>% 
          gsub("\\.", "_", .)
maf_out$name <- tmpsnp
names(maf_out) <- c("SNP", "Alt Allele Freq")

saveRDS(maf_out, file = paste0("/media/work/tmp/posthoc/", env, "/allelefreq_", env, ".rds"), version = 2)
...
# plot wrap function. Outcome is ALWAYS ALWAYS ALWAYS 'outcome'
plot_wrapper <- function(dat, exposure, snp, covariates = c()) {
  
  # check if exposure variable is binary, fit models accordingly (factor vs numeric)
  ...
  tmp_model <- paste0("outcome ~ ", snp_rename, "+", exposure, "+", paste0(covariates, collapse = "+"))
  tmp_coeff <- coef(lm(tmp_model, data = tmp))[2]
  if(tmp_coeff < 0) {
    tmp[, snp_rename] <- abs(tmp[, snp_rename] - 2)
  }
  ...
  png(paste0("/media/work/tmp/posthoc/", exposure, "/effects_plot_", exposure, "_", snp_rename, "_", paste0(sort(covariates), collapse = "_"), "_e.png"), height = 4, width = 5, units = 'in', res = 150)
  print({
    plot(model_eff, snp_rename, multiline = T, confint = list(style = 'bands'), type = 'link', ylab = 'predicted outcome log-odds')
  })
  dev.off()
  
  png(paste0("/media/work/tmp/posthoc/", exposure, "/effects_plot_", exposure, "_", snp_rename, "_", paste0(sort(covariates), collapse = "_"), "_g.png"), height = 4, width = 5, units = 'in', res = 150)
  print({
...
sapply(snp_list, function(x) create_stratified_or_table_yi(dat = dat, env = env, snps=x, covs = covs, mod = mod))
