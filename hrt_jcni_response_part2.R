...
# 05/22/2022
# 
# convert some forest plots to eps or ai format for editors
#=============================================================================#

#-----------------------------------------------------------------------------# 
# association between HRT and other variables with CRC (main effects)
...
# ----- MHT (main effects by study design) -----
...
meta_input <- get_counts_outcome_by_group(input_data, "outcome", "study_gxe") %>%
  mutate(study_gxe = as.character(study_gxe)) %>% 
  inner_join(study_design, "study_gxe") %>% 
  full_join(glm_out, "study_gxe")

class(meta_input$study_design)
table(meta_input$study_design)
...
setEPS()
postscript("~/Figure_1.eps", height = 13, width = 10)
...
meta::forest(results_meta, 
             ...
             # xlim = c(0.25, 4),
             layout = "JAMA",
             resid.hetstat = T, print.I2 = T,
             #test.overall.fixed = T,
...
setEPS()
postscript("~/Figure_3.eps", height = 9, width = 10)
...
setEPS()
postscript("~/Figure_2.eps", height = 9, width = 10)
