...
vars_to_keep <- c("vcfid", "outcome", "methrswklns", "age_ref_imp", "sex", "study_gxe", "energytot_imp", "PC1", "PC2", "PC3")
tmp <- figi_gxe %>%
  dplyr::filter(gxe == 1,
                EUR_subset == 1,
                !is.na(methrswklns)) %>% 
...
saveRDS(tmp, file = glue("/media/work/gwis_test/methrswklns/data/FIGI_v3.1_gxeset_methrswklns_basic_covars_glm.rds"))

cov_gxescan <- format_data_gxescan(tmp, 'methrswklns')
saveRDS(cov_gxescan, file = glue("/media/work/gwis_test/methrswklns/data/FIGI_v3.1_gxeset_methrswklns_basic_covars_gxescan.rds"), version = 2)

x <- readRDS(glue("/media/work/gwis_test/methrswklns/data/FIGI_v3.1_gxeset_methrswklns_basic_covars_glm.rds"))
table(x$study_gxe, x$outcome)
y <- readRDS(glue("/media/work/gwis_test/methrswklns/data/FIGI_v3.1_gxeset_methrswklns_basic_covars_gxescan.rds"))
head(y)
...
# ------ methrswklnsqc2 N = 42602 ------ #

#methrswklnsqc2
load("/media/work/gwis_test/data/physact_additional_vars/PA_Dataset.Rdata")

# Does N match between methrswklns and methrswklnsqc2 - YES
nrow(filter(gxe, !is.na(methrswklns)))
nrow(filter(gxe, !is.na(methrswklnsqc2)))
...
vars_to_keep <- c("vcfid", "outcome", "methrswklnsqc2", "age_ref_imp", "sex", "study_gxe", "energytot_imp", "PC1", "PC2", "PC3")
tmp <- figi_gxe %>% dplyr::select(-methrswklnsqc2) %>%
  inner_join(gxe[,c('vcfid', 'methrswklnsqc2')], 'vcfid') %>%
  dplyr::filter(gxe == 1,
                EUR_subset == 1,
                !is.na(methrswklnsqc2)) %>% 
  mutate(outcome = fct_drop(outcome))
...
# I don't think need to drop studies with missing categories 
# (like in dietary variables where some studies only have q2,q3 vs q1-q4)
# table(tmp$methrswklnsqc2, tmp$study_gxe)
# 
# drops_e <- data.frame(table(tmp$methrswklnsqc2, tmp$study_gxe)) %>%
#   filter(Freq <= 0)
...
tmp <- filter(tmp, !study_gxe %in% unique(drops$Var2)) %>%
  ...
  dplyr::filter(!study_gxe %in% exclude) %>% 
  dplyr::mutate(study_gxe = fct_drop(study_gxe), 
                methrswklnsqc2 = as.numeric(methrswklnsqc2), 
                outcome = ifelse(outcome == "Control", 0, 1),
                sex = ifelse(sex == "Female", 0, 1))
...
saveRDS(tmp, file = glue("/media/work/gwis_test/methrswklnsqc2/data/FIGI_v3.1_gxeset_methrswklnsqc2_basic_covars_glm.rds"))

cov_gxescan <- format_data_gxescan(tmp, 'methrswklnsqc2')
saveRDS(cov_gxescan, file = glue("/media/work/gwis_test/methrswklnsqc2/data/FIGI_v3.1_gxeset_methrswklnsqc2_basic_covars_gxescan.rds"), version = 2)

x <- readRDS(glue("/media/work/gwis_test/methrswklnsqc2/data/FIGI_v3.1_gxeset_methrswklnsqc2_basic_covars_glm.rds"))
table(x$study_gxe, x$outcome)
table(x$outcome, x$methrswklnsqc2)
y <- readRDS(glue("/media/work/gwis_test/methrswklnsqc2/data/FIGI_v3.1_gxeset_methrswklnsqc2_basic_covars_gxescan.rds"))
head(y)
...
tmp <- figi_gxe %>%
  ...
  dplyr::filter(gxe == 1,
                ...
                !is.na(methrswklns)) %>% 
  mutate(outcome = fct_drop(outcome), 
         active_met_1800 = ifelse(is.na(methrswklns),NA,
                                 ifelse(methrswklns>=18,'Yes','No')))
...
vars_to_keep <- c("vcfid", "outcome", "active_met_1800", "age_ref_imp", "sex", "energytot_imp", "study_gxe", "PC1", "PC2", "PC3")
...
tmp <- filter(tmp, !study_gxe %in% unique(drops$Var2)) %>%
  ...
  dplyr::filter(!study_gxe %in% exclude) %>% 
  dplyr::mutate(study_gxe = fct_drop(study_gxe), 
                active_met_1800 = as.numeric(ifelse(active_met_1800 == "No", 0, 1)), 
                outcome = ifelse(outcome == "Control", 0, 1),
                sex = ifelse(sex == "Female", 0, 1))



table(tmp$active_met_1800, tmp$study_gxe)
drops_e <- data.frame(table(tmp$active_met_1800, tmp$study_gxe)) %>% 
  filter(Freq <= 0)
...
saveRDS(tmp, file = glue("/media/work/gwis_test/active_met_1800/data/FIGI_v3.1_gxeset_active_met_1800_basic_covars_glm.rds"))

cov_gxescan <- format_data_gxescan(tmp, 'active_met_1800')
saveRDS(cov_gxescan, file = glue("/media/work/gwis_test/active_met_1800/data/FIGI_v3.1_gxeset_active_met_1800_basic_covars_gxescan.rds"), version = 2)

x <- readRDS(glue("/media/work/gwis_test/active_met_1800/data/FIGI_v3.1_gxeset_active_met_1800_basic_covars_glm.rds"))
table(x$study_gxe, x$outcome)
table(x$outcome, x$active_met_1800)
y <- readRDS(glue("/media/work/gwis_test/active_met_1800/data/FIGI_v3.1_gxeset_active_met_1800_basic_covars_gxescan.rds"))
head(y)
...
tmp <- figi_gxe %>%
  ...
  dplyr::filter(gxe == 1,
                ...
                !is.na(methrswklns)) %>% 
  mutate(outcome = fct_drop(outcome), 
         active_met_3500 = ifelse(is.na(methrswklns),NA,
                                  ifelse(methrswklns>=35,'Yes','No')))
...
vars_to_keep <- c("vcfid", "outcome", "active_met_3500", "age_ref_imp", "sex", "study_gxe", "PC1", "PC2", "PC3")
...
tmp <- filter(tmp, !study_gxe %in% unique(drops$Var2)) %>%
  ...
  dplyr::filter(!study_gxe %in% exclude) %>% 
  dplyr::mutate(study_gxe = fct_drop(study_gxe), 
                active_met_3500 = as.numeric(ifelse(active_met_3500 == "No", 0, 1)), 
                outcome = ifelse(outcome == "Control", 0, 1),
                sex = ifelse(sex == "Female", 0, 1))
...
saveRDS(tmp, file = glue("/media/work/gwis_test/active_met_3500/data/FIGI_v3.1_gxeset_active_met_3500_basic_covars_glm.rds"))

cov_gxescan <- format_data_gxescan(tmp, 'active_met_3500')
saveRDS(cov_gxescan, file = glue("/media/work/gwis_test/active_met_3500/data/FIGI_v3.1_gxeset_active_met_3500_basic_covars_gxescan.rds"), version = 2)

x <- readRDS(glue("/media/work/gwis_test/active_met_3500/data/FIGI_v3.1_gxeset_active_met_3500_basic_covars_glm.rds"))
table(x$study_gxe, x$outcome)
table(x$outcome, x$active_met_3500)
y <- readRDS(glue("/media/work/gwis_test/active_met_3500/data/FIGI_v3.1_gxeset_active_met_3500_basic_covars_gxescan.rds"))
head(y)
