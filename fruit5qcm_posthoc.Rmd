...
snps_out <- c("chr1_72729142_A_G")

# Call child Rmd script
res <- lapply(snps_out, function(snp) {
  assign("snp", snp, child_env)
  knitr::knit_child('./posthoc/posthoc_child_scaled.Rmd', envir = child_env, quiet = TRUE)
  }
)
...
assign("exposure", params$exposure, child_env)
assign("method", "chiSqGxE", child_env)
assign("covariates", sort(covariates), child_env)
```

```{r, results = 'asis'}
# snps_out <- readRDS(glue(path, "twostep_wht_chiSqGE_{params$exposure}_expectation_hybrid_df.rds")) %>%
#   arrange(Chromosome, Location) %>%
#   dplyr::mutate(snps = paste0('chr', gsub("\\:", "\\_", SNP))) %>%
#   pull(snps)

snps_out <- c("chr8_117689668_T_C", "chr8_117742957_A_C", "chr8_117752713_C_T")

res <- lapply(snps_out, function(snp) {
  assign("snp", snp, child_env)
  knitr::knit_child(
    glue('posthoc/posthoc_child_scaled.Rmd'), envir = child_env, quiet = TRUE)
  }
)
cat(unlist(res), sep = '\n')
```
